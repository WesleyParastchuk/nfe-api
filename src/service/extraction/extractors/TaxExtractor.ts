...
export function extractTaxes($: CheerioAPI): TaxDTO {
  ...
  const federalMatch = taxInfoText.match(/Trib aprox R\$:\s*([\d,]+)\s*Federal/);
  const stateMatch = taxInfoText.match(/R\$:\s*([\d,]+)\s*Estadual/);

  const federal = federalMatch ? parseFloat(federalMatch[1].replace(",", ".")) : 0;
